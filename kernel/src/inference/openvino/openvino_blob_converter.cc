...
namespace rayshape
{
    namespace openvino
    {
        ...
        std::shared_ptr<ov::Tensor> OpenvinoBlobConverter::ConvertFromBlob(ErrorCode &status,
            ...
            status =
                OpenVINOConfigConverter::ConvertFromDataType(openvino_precision, src->data_type);
            ...
            status =
                OpenVINOConfigConverter::ConvertFromDataFormat(openvino_layout, src->data_format);
            if (status != RS_SUCCESS) {
        ...
        ErrorCode OpenvinoBlobConverter::CreateOrUpdateBlob(Blob **dst, const ov::Tensor &src,
            ...
            if (alloc) {
                ...
                RSMemoryInfo mem_info{ mem_type, blob->data_type, size };
