...
namespace rayshape
{
    namespace utils
    {
        ...
        struct ProcedureTimePoint {
            ...
            ProcedureTimePoint(std::string name_procedure, ProcedureFlag flag) :
        ...
        private:
            ...
            void UpdataProcedureElapsedMap(
                std::map<std::string, std::shared_ptr<ProcedureElapsed>> &time_elapsed,
                std::shared_ptr<ProcedureTimePoint> &p_tp_start,
                std::shared_ptr<ProcedureTimePoint> &p_tp_end, std::string full_name, int level,
                int order);

            void Write2TimeLogCSV(std::string name_file,
        ...
        void TimeProfile::InsertProcedureTimePoint(std::string name_device,
                                                   std::string name_procedure, ProcedureFlag flag) {
            std::chrono::time_point<std::chrono::system_clock> tp =
                std::chrono::system_clock::now();
        ...
        void TimeProfile::UpdataProcedureElapsedMap(
            std::map<std::string, std::shared_ptr<ProcedureElapsed>> &time_elapsed,
            std::shared_ptr<ProcedureTimePoint> &p_tp_start,
            std::shared_ptr<ProcedureTimePoint> &p_tp_end, std::string full_name, int level,
            int order) {
            float delta =
                duration_cast<microseconds>(p_tp_end->tp_ - p_tp_start->tp_).count() / 1000.0f;
            ...
            if (time_elapsed.find(full_name) == time_elapsed.end()) {
                ...
                auto p_e = std::make_shared<ProcedureElapsed>(
                    name_device, p_tp_start->name_procedure_, level, 1, order, delta);
                time_elapsed.insert(
                    std::pair<std::string, std::shared_ptr<ProcedureElapsed>>(full_name, p_e));
            } else {
        ...
        int TimeProfile::DownloadTimeProfile(std::string name_file) {
            ...
            for (auto ite_p_tp : p_tp_vec_) {
                ...
                if (PROCEDURE_FLAG_START == ite_p_tp->flag_) {
                    ...
                    if (start->name_procedure_ == ite_p_tp->name_procedure_) {
                        UpdataProcedureElapsedMap(time_elapsed, start, ite_p_tp, full_name, level,
                                                  order_map[full_name]);
                        match_vec.pop_back();
                    } else {
                        // TODO
            ...
            struct {
                bool operator()(std::shared_ptr<ProcedureElapsed> lhs,
                                std::shared_ptr<ProcedureElapsed> rhs) const {
        ...
        // void InitTimeProfile(std::string name_compiler, std::string name_os, std::string
        // name_host) {
        ...
        void InsertProcedureTimePointStart(std::string name_device, std::string name_procedure) {
            time_profile->InsertProcedureTimePoint(name_device, name_procedure,
                                                   PROCEDURE_FLAG_START);
        }
