    ...
    namespace dag
    {
        ...
        std::vector<Edge *> Node::Forward(std::vector<Edge *> inputs) {
            ...
            std::vector<std::string> real_outputs_name =
                this->GetRealOutputsName(); // 针对根据type信息构造边
            std::vector<Edge *> outputs;
            // 节点operation.
        ...
        void Node::SetRunningFlag(bool flag) {
            ...
            is_running_ = flag;
            // insert time point start.
            // 两种方式一种设置，第二种在每个节点里插入NNDEPLOY_TIME_POINT_START
            // if (is_time_profile_) {
            //     if (is_running_) {
            //         INSERT_TIME_POINT_START("CPU", node_name_ + " run()");
            //     } else {
            //         INSERT_TIME_POINT_END("CPU", node_name_ + " run()");
            //     }
            // }
        }
